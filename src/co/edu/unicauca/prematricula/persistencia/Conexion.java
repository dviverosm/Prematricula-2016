...
public class Conexion implements HttpSessionBindingListener {
	...
	/**
	 * Mótodo encargado de ejecutar la sentencia SQL. 
	 * @param sql Contiene la sentencia SQL que se va a ejecutar.
	 * @param parameters Contiene los parómetros de la consulta que se desea ejecutar
	...
	public ResultSet executeQueryRSET(String sql, ArrayList parameters) {
		// Se llama al mótodo encargado de iniciar el datasource
		this.loadDataSource();
		ResultSet rset = null;
		try {
			...
			if (parameters.size() > 0)
				for (int i = 0; i < parameters.size(); i++) {
					// Se establece el string de parómetros de la consulta
    ...
    * se invoca el mótodo para iniciar el datasource.
    * @return Retorna la conexión a la base de datos.
    */
	public Connection getConexion() {
		try {
			// Si no se ha inicializado la conexión, se invoca el mótodo para iniciar el datasource
	...
	/**
	 * Mótodo utilizado para cerrar la conexión a la base de datos
